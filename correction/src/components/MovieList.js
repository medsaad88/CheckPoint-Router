...
const MovieList = () => {
  ...
  return <Link to={{pathname:`/movie/${movie.id}?title=${movie.title}&description=${movie.description}&posterUrl=${movie.posterUrl}&rate=${movie.rate}&trailler=${movie.trailler}`,state:movie}} key={movie.id} ><MovieCard movie={movie}  /></Link>;
      });
    } else {
      return (
